%%%%%%%%%%%%%%%%%%%%
% Test l'optimisation SMO, cas "SVM 2 classes"
% SVM 2 classes - avec noyau autre que le produit scalaire
% - Optimisation avec la méthode SMO
% - Comparaison avec ce qu'on obtient avec LOQO
...
switch numexemple
  ...
  case 3
    ...
    moy1=mean([cl1_d1 cl2_d1]);
    std1=std([cl1_d1 cl2_d1]);

    moy2=mean([cl1_d2 cl2_d2]);
    std2=std([cl1_d2 cl2_d2]);
    ...
    %%% classe 2
    cl2_d1 = (don2(1:nn,1)'-moy1)/std1;
    cl2_d2 = (don2(1:nn,2)'-moy2)/std2;
...
%%% calcul de la solution : optimisation sous contraintes, méthode SMO
ttsmo=clock;
...
[alpha, bbbopt] = lagis_smo_2classes(donnees, classes, 1e3, sig2, ttsmo);
eesmo=etime(clock,ttsmo);

alphax = alpha(1:nn)';        % alphas pour la première classe
alphay = alpha(nn+1:2*nn)';   % alphas pour la deuxième classe

figure(1);
clf;
subplot(211);
title('alphas obtenus avec SMO');
grid on;
hold on;
plot(alphax,'r');
hold off;
...
%%%%%%%%%%%%%%%%%%%%%
%%% on compare à ce qu'on obtient avec LOQO !!!!
...
fprintf(1,'temps requis pour SMO : %f\n',eesmo);
if (eeloqo>0)
  fprintf(1,'temps requis pour LOQO : %f\n',eeloqo);
else
  fprintf(1,'LOQO n est pas calcule\n');
end;
...
%%% plot pour la route - SMO
vssmocl1 = alphax>max(alpha)/20.0;
vssmocl2 = alphay>max(alpha)/20.0;
figure(5);
clf;
grid on;
hold on;
title('les points magentas sont les vecteurs supports trouves par SMO')
plot(cl1_d1,cl1_d2,'*r');
plot(cl2_d1,cl2_d2,'ob');
for ii=1:nn
  if (vssmocl1(ii)>0)
